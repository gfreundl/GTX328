...
008				cold&dark for Avionics via deferred init
009				smooth lead-in fuel gauges
				lead-out coded, but inop as gauges are hardwired to bat in panel, quite complex to reset actual fuel state
010				fixed fuel pump sound 
				clean up abandonned fuel gauge lead-out residues
				TBQ:	CommandRefs too restricted for toggle switches, read only changes and ignore stable switch states
						Fuel pump, Battery and Avionics switches  back to joystick readout, too many problems with CommandRefs
				
011				Fuel Pump switch reverted back to CommandRef
				tbq, why CommandRefs not suited
				dual Fuel Pump switches, as A/C Reload turns all pumps on
				created custom DataRefs for importing BFF Trim settings via XPUIPC offset, to show indicate positions in panels;
				added callback for DRE export of custom DataRefs

				temp support for toe brakes, would only work with analog yaw pot input

012				on failure disable fuel pump, ldg light, strobes, trim, pitot heat, nav light, audio, flaps
				annunciator_pressed DataRef not useable: includes autopilot annunciators, timeout shows random timer
				Custom Command gfe_annunciator_test_timeout created for use in panel, custom DataRefs to be added DataRefs.txt for PlaneMaker

013				some inits (brakes, fuel_pump) on every aircraft reload
				HSI slaved/free gyro function



Open Issues
adjust throttle scaling ? <- to what scaling function?
rename CommandRefs to copy description, as search functions parses only name, not description

Consider reverting back to joystick values, at least for this plugin
reconsider joystick revert
...
XPLMDataRef d_joy_mapped_axis_value = NULL;
...
XPLMDataRef d_fuel_pump_spinning = NULL;
...
XPLMDataRef d_total_flight_time_sec = NULL;
...
XPLMDataRef d_avionics_power_on = NULL;											//switch
XPLMDataRef d_avionics_EQ = NULL;
XPLMDataRef d_m_fuel = NULL;
XPLMDataRef d_gfe_elev_trim = NULL;												//custom Datarefs for BFF
XPLMDataRef d_gfe_ail_trim = NULL;
XPLMDataRef d_gfe_rud_trim = NULL;
XPLMDataRef d_override_toe_brakes = NULL;
XPLMDataRef d_rel_ele_fuepmp0 = NULL;
XPLMDataRef d_rel_lites_land = NULL;
XPLMDataRef d_rel_lites_strobe = NULL;
XPLMDataRef d_rel_trim_elv = NULL;
XPLMDataRef d_rel_ice_pitot_heat1 = NULL;
XPLMDataRef d_rel_lites_nav = NULL;
XPLMDataRef d_rel_marker = NULL;
XPLMDataRef d_rel_gear_warning = NULL;
XPLMDataRef d_rel_gear_ind = NULL;
XPLMDataRef d_rel_gear_act = NULL;
XPLMDataRef d_rel_xpndr = NULL;
XPLMDataRef d_rel_flap_act = NULL;
XPLMDataRef d_gfe_annunciators = NULL;											//custom DR for annunciators light up with timeout
XPLMDataRef d_dg_drift_ele_deg = NULL;
XPLMDataRef d_gyr_free_slaved = NULL;
...
XPLMCommandRef gfe_fuel_pump1 = NULL;
XPLMCommandRef gfe_fuel_pump2 = NULL;
...
XPLMCommandRef gfe_gyr_free_slaved = NULL;
XPLMCommandRef gfe_dg_drift_ele_deg_up = NULL;
XPLMCommandRef gfe_dg_drift_ele_deg_down = NULL;

int OLDjoystick_button_values[1600] = { 0 };
int joystick_button_values[1600] = { 0 };
float joystick_axis_values[500] = { 0.0 };
float joy_mapped_axis_value[500] = { 0.0 };
int fuel_pump;
int battery_on;
...
int annunciator_test_lit;														//for custom Dataref
int fire_mode;
...
int adf2_power;
int flaps_timer = 0;
int fuel_gauge_flag = 0;														//1 = fuel gauge lead-in, = lead-out
float gauge_leadin_ratio = 0.0;
float mem_m_fuel[9] = { 0.0 };													//memorize original fuel weight
int FuelPump[8] = {0};															
int FuelPumpOld[8] = {0};
float fuel_pump_spinning[8] = { 0 };
int i = 0;
int n = 0;
int FLCB = 0;																	//Flight Loop Counter
float elev_trim_ind = 0.0;														//for custom DataRef
float ail_trim_ind = 0.0;
float rud_trim_ind = 0.0;
int dg_drift_ele_deg;
int gyr_free_slaved[6] = { 0 };

char gPluginDataFile[255];														//file path and name
DWORD timestamp;																//timer for KAP150 test sequence
DWORD timestamp_annunciators = 0;												//timer for annunciators
...
int gfe_fuel_pump1_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon);
int gfe_fuel_pump2_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon);
...
int gfe_gyr_free_slaved_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon);
int gfe_dg_drift_ele_deg_up_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon);
int gfe_dg_drift_ele_deg_down_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon);

float Get_d_gfe_elevtrimCB(void* inRefcon);
void Set_d_gfe_elevtrimCB(void* inRefcon, float inValue);
float Get_d_gfe_ailtrimCB(void* inRefcon);
void Set_d_gfe_ailtrimCB(void* inRefcon, float inValue);
float Get_d_gfe_rudtrimCB(void* inRefcon);
void Set_d_gfe_rudtrimCB(void* inRefcon, float inValue);
int Get_d_gfe_annunciatorCB(void* inRefcon);
void Set_d_gfe_annunciatorCB(void* inRefcon, int inValue);

float CustomDatarefToDRECB(float elapsedMe, float elapsedSim, int counter, void * refcon);
void smooth_fuel_gauge_Proc();
...
PLUGIN_API int XPluginStart(char * name, char * sig, char * desc)
{
	...
	d_joy_mapped_axis_value = XPLMFindDataRef("sim/joystick/joy_mapped_axis_value");
	...
	d_fuel_pump_spinning = XPLMFindDataRef("sim/flightmodel2/engines/fuel_pump_spinning");
	...
	d_total_flight_time_sec = XPLMFindDataRef("sim/time/total_flight_time_sec");
	...
	d_avionics_power_on = XPLMFindDataRef("sim/cockpit2/switches/avionics_power_on");
	d_avionics_EQ = XPLMFindDataRef("sim/cockpit/electrical/avionics_EQ");
	d_m_fuel = XPLMFindDataRef("sim/flightmodel/weight/m_fuel");
	d_override_toe_brakes = XPLMFindDataRef("sim/operation/override/override_toe_brakes");
	d_rel_ele_fuepmp0 = XPLMFindDataRef("sim/operation/failures/rel_ele_fuepmp0");
	d_rel_flap_act = XPLMFindDataRef("sim/operation/failures/rel_gear_ind");
	...
	d_gyr_free_slaved = XPLMFindDataRef("sim/cockpit/gyros/gyr_free_slaved");

	//Custom DataRef for Trim Indicator and Annunciators
	d_gfe_elev_trim = XPLMRegisterDataAccessor("gfe/elevtrimindicator", xplmType_Float, 1, NULL, NULL, Get_d_gfe_elevtrimCB, Set_d_gfe_elevtrimCB, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	d_gfe_elev_trim = XPLMFindDataRef("gfe/elevtrimindicator");
	d_gfe_ail_trim = XPLMRegisterDataAccessor("gfe/ailtrimindicator", xplmType_Float, 1, NULL, NULL, Get_d_gfe_ailtrimCB, Set_d_gfe_ailtrimCB, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	d_gfe_ail_trim = XPLMFindDataRef("gfe/ailtrimindicator");
	d_gfe_rud_trim = XPLMRegisterDataAccessor("gfe/rudtrimindicator", xplmType_Float, 1, NULL, NULL, Get_d_gfe_rudtrimCB, Set_d_gfe_rudtrimCB, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	d_gfe_rud_trim = XPLMFindDataRef("gfe/rudtrimindicator");
	XPLMSetDataf(d_gfe_elev_trim, 0.0);
	XPLMSetDataf(d_gfe_ail_trim, 0.0);
	XPLMSetDataf(d_gfe_rud_trim, 0.0);
	d_gfe_annunciators = XPLMRegisterDataAccessor("gfe/annunciators", xplmType_Int, 1, Get_d_gfe_annunciatorCB, Set_d_gfe_annunciatorCB, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	d_gfe_annunciators = XPLMFindDataRef("gfe/annunciators");
	XPLMSetDatai(d_gfe_annunciators, 0);
	...
	gfe_fuel_pump1 = XPLMCreateCommand("gfe/Engine/fuel_pump1", "Fuel Pump 1");
	gfe_fuel_pump2 = XPLMCreateCommand("gfe/Engine/fuel_pump2", "Fuel Pump 2");
	...
	gfe_gyr_free_slaved = XPLMCreateCommand("gfe/Avionics/gyr_free_slaved", "KA51 Free/Slaved");
	gfe_dg_drift_ele_deg_up = XPLMCreateCommand("gfe/Avionics/dg_drift_ele_deg_up", "KA51 CW");
	gfe_dg_drift_ele_deg_down = XPLMCreateCommand("gfe/Avionics/dg_drift_ele_deg_down", "KA51 CCW");
	...
	XPLMRegisterCommandHandler(gfe_fuel_pump1, gfe_fuel_pump1_Proc, 1, (void*)0);
	XPLMRegisterCommandHandler(gfe_fuel_pump2, gfe_fuel_pump2_Proc, 1, (void*)0);
	...
	XPLMRegisterCommandHandler(gfe_gyr_free_slaved, gfe_gyr_free_slaved_Proc, 1, (void*)0);
	XPLMRegisterCommandHandler(gfe_dg_drift_ele_deg_up, gfe_dg_drift_ele_deg_up_Proc, 1, (void*)0);
	XPLMRegisterCommandHandler(gfe_dg_drift_ele_deg_down, gfe_dg_drift_ele_deg_down_Proc, 1, (void*)0);
	...
	//Callback for registering Custom DataRefs in DRE, defer until plugins loaded
	XPLMRegisterFlightLoopCallback(CustomDatarefToDRECB, 1, NULL);

	//Register FlightLoop Callback for every 100msec
	XPLMRegisterFlightLoopCallback(
		MyFlightLoopCallback,	/* Callback */
		0.10,					/* Interval */
		NULL);					/* refcon not used. */

	//support for toe brakes
	XPLMSetDatai(d_override_toe_brakes, 1);
	return 1;
}
...
PLUGIN_API void XPluginStop(void)
{
	...
	XPLMUnregisterDataAccessor(d_gfe_elev_trim);
	XPLMUnregisterDataAccessor(d_gfe_ail_trim);
	XPLMUnregisterDataAccessor(d_gfe_rud_trim);
	XPLMUnregisterDataAccessor(d_gfe_annunciators);
	XPLMUnregisterFlightLoopCallback(CustomDatarefToDRECB, NULL);
	...
	XPLMUnregisterCommandHandler(gfe_fuel_pump1, gfe_fuel_pump1_Proc, 0, 0);
	XPLMUnregisterCommandHandler(gfe_fuel_pump2, gfe_fuel_pump2_Proc, 0, 0);
	...
	XPLMUnregisterCommandHandler(gfe_gyr_free_slaved, gfe_gyr_free_slaved_Proc, 0, 0);
	XPLMUnregisterCommandHandler(gfe_dg_drift_ele_deg_up, gfe_dg_drift_ele_deg_up_Proc, 0, 0);
	XPLMUnregisterCommandHandler(gfe_dg_drift_ele_deg_down, gfe_dg_drift_ele_deg_down_Proc, 0, 0);

	XPLMSetDatai(d_override_toe_brakes, 0);
	...
	void *               inRefcon)
{
	...
	//some one-off DataRefs, not all would work in XPLMPluginStart (d_avionics_power_on)
	//runs on any aircraft reload too (reload resets total_flight_time_sec to 0)
	if ((FLCB == 1) || (XPLMGetDataf(d_total_flight_time_sec) < 0.5))
	{
		XPLMSetDataf(d_parking_brake_ratio, 0.0);									//unset parking brake
		//XPLMSetDatai(d_avionics_power_on, 0);										//requires deferred init
		XPLMSetDatai(d_battery_on, 0);												//default to cold & dark
		//XPLMSetDatai(d_avionics_EQ, 1);												//Avionics Switch exists
	...
	//read all joystick axis values and assigments
	i = XPLMGetDatavf(d_joystick_axis_values, joystick_axis_values, 0, 500);		//raw, uncalibrated data
	i = XPLMGetDatavf(d_joy_mapped_axis_value, joy_mapped_axis_value, 0, 500);		//mapped, calibrated, referenced by joystick_axis_assignments
	...
	//support for toe brakes when no rudder pot is calibrated
	XPLMSetDataf(d_left_brake_ratio, joy_mapped_axis_value[6]);
	XPLMSetDataf(d_right_brake_ratio, joy_mapped_axis_value[7]);
	...
	//Battery Switch
	//another try with CommandRef
	/*
	XPLMSetDatai(d_battery_on, joystick_button_values[983]);
	if ((joystick_button_values[983] == 1) && (OLDjoystick_button_values[983] == 0))
	{
		XPLMSetDatai(d_fire_mode, 3);											//on Battery on, bring KAP150 in pre-test mode
		XPLMGetDatavf(d_m_fuel, mem_m_fuel, 0, 9);								//save current fuel state
		fuel_gauge_flag = 1;													//smooth lead-in fuel gauge
		gauge_leadin_ratio = 0.0;
	}
	if (joystick_button_values[983] == 1)
	{
		smooth_fuel_gauge_Proc();
	}
	else
	{
		gauge_leadin_ratio = 0.0;
	...
	*/
	...
	//Avionics Switch
	XPLMSetDatai(d_avionics_power_on, joystick_button_values[971]);
	...
	//fuel pump switch
	//FuelPump[0] = joystick_button_values[8];
	//XPLMSetDatavi(d_fuel_pump, FuelPump, 0, 8);

	//play pump sound when pump runs, currently only one pump used
	...
	//light up annunciators for testing when triggered by CustomCommand
	if (GetTickCount() - timestamp_annunciators < 4000)
	{
		annunciator_test_lit = 1;
	}
	else
	{
		annunciator_test_lit = 0;
	}
...
int gfe_hsi_obs_10_up_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandEnd)
	{
		if ((XPLMGetDataf(d_nav1_obs_deg_mag_pilot) < 350.0) || (XPLMGetDataf(d_hsi_obs_deg_mag_pilot) < 350.0))
		{
			XPLMSetDataf(d_nav1_obs_deg_mag_pilot, XPLMGetDataf(d_nav1_obs_deg_mag_pilot) + 10.0);
			XPLMSetDataf(d_hsi_obs_deg_mag_pilot, XPLMGetDataf(d_hsi_obs_deg_mag_pilot) + 10.0);
		}
		else
		{
			XPLMSetDataf(d_nav1_obs_deg_mag_pilot, 0.0);
...
int gfe_hsi_obs_10_down_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandEnd)
	{
		if ((XPLMGetDataf(d_nav1_obs_deg_mag_pilot) >  10.0) || (XPLMGetDataf(d_hsi_obs_deg_mag_pilot) >  10.0))
		{
			XPLMSetDataf(d_nav1_obs_deg_mag_pilot, XPLMGetDataf(d_nav1_obs_deg_mag_pilot) - 10.0);
			XPLMSetDataf(d_hsi_obs_deg_mag_pilot, XPLMGetDataf(d_hsi_obs_deg_mag_pilot) - 10.0);
		}
...
int gfe_hsi_hdg_10_up_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandBegin || inPhase == xplm_CommandContinue)
	{
		if (XPLMGetDataf(d_heading_dial_deg_mag_pilot) < 350.0)
		{
			XPLMSetDataf(d_heading_dial_deg_mag_pilot, XPLMGetDataf(d_heading_dial_deg_mag_pilot) + 10.0);
		}
		else
		{
			XPLMSetDataf(d_heading_dial_deg_mag_pilot, 0.0);
...
int gfe_hsi_hdg_10_down_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandBegin || inPhase == xplm_CommandContinue)
	{
		if (XPLMGetDataf(d_heading_dial_deg_mag_pilot) >  10.0)
		{
			XPLMSetDataf(d_heading_dial_deg_mag_pilot, XPLMGetDataf(d_heading_dial_deg_mag_pilot) - 10.0);
		}
		else
...
int gfe_obs2_10_up_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandEnd)
	{
		if (XPLMGetDataf(d_nav2_obs_deg_mag_pilot) < 350.0)
		{
			XPLMSetDataf(d_nav2_obs_deg_mag_pilot, XPLMGetDataf(d_nav2_obs_deg_mag_pilot) + 10.0);
		}
		else
		{
			XPLMSetDataf(d_nav2_obs_deg_mag_pilot, 0.0);
...
int gfe_obs2_10_down_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandEnd)
	{
		if (XPLMGetDataf(d_nav2_obs_deg_mag_pilot) >  10.0)
		{
			XPLMSetDataf(d_nav2_obs_deg_mag_pilot, XPLMGetDataf(d_nav2_obs_deg_mag_pilot) - 10.0);
		}
		else
...
int gfe_rmi_10_up_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandEnd)
	{
		if (XPLMGetDataf(d_adf1_card_heading_deg_mag_pilot) < 350.0)
		{
			XPLMSetDataf(d_adf1_card_heading_deg_mag_pilot, XPLMGetDataf(d_adf1_card_heading_deg_mag_pilot) + 10.0);
		}
		else
		{
			XPLMSetDataf(d_adf1_card_heading_deg_mag_pilot, 0.0);
...
int gfe_rmi_10_down_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandEnd)
	{
		if (XPLMGetDataf(d_adf1_card_heading_deg_mag_pilot) >  10.0)
		{
			XPLMSetDataf(d_adf1_card_heading_deg_mag_pilot, XPLMGetDataf(d_adf1_card_heading_deg_mag_pilot) - 10.0);
		}
		else
...
int	gfe_ext_bff_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	...
	strcpy(str, "\"E:\\BFF_Control_Loader_v1_322_Release 3\\Configuration Manager\\Aircraft Config Files\\");
	strcat(str, acf_ICAO);
	strcat(str, ".cfg\"");
	if (inPhase == xplm_CommandEnd)
	{
		int nRet = (int)ShellExecute(NULL, "open",
			"\"E:\\BFF_Control_Loader_v1_322_Release 3\\BFF_Control_LoaderX_v1_322.exe\"", str ,	NULL, SW_SHOWNORMAL);
		if (nRet <= 32) 
		{
...
int gfe_flap_ratio_up_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (XPLMGetDatai(d_rel_flap_act) == 0)
	{
...
int gfe_battery_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandBegin && XPLMGetDatai(d_battery_on) == 0)
	{
	...
	else if (inPhase == xplm_CommandContinue && XPLMGetDatai(d_battery_on) == 1)
	{
		smooth_fuel_gauge_Proc();
	}
	else if (inPhase == xplm_CommandEnd && XPLMGetDatai(d_battery_on) == 1)
	{
		XPLMSetDatai(d_battery_on, 0);
		fuel_gauge_flag = 0;													//stop smooth lead-in fuel gauge
		gauge_leadin_ratio = 0.0;
	}
	else
	{
...
int gfe_fuel_pump1_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	...
	if (inPhase == xplm_CommandBegin && FuelPump[0] == 0 && XPLMGetDatai(d_battery_on) && !XPLMGetDatai(d_rel_ele_fuepmp0))
	{
		FuelPump[0] = 1;
	}
	else if (inPhase == xplm_CommandContinue && FuelPump[0] == 0 && XPLMGetDatai(d_battery_on) && !XPLMGetDatai(d_rel_ele_fuepmp0))
	{
		FuelPump[0] = 1;
	}
	else if (inPhase == xplm_CommandEnd || XPLMGetDatai(d_battery_on) == 0 || XPLMGetDatai(d_rel_ele_fuepmp0)) 
	{
		FuelPump[0] = 0;
	}
	XPLMSetDatavi(d_fuel_pump, FuelPump, 0, 8);
...
{
	//int a[8] = { 0 };															//temp array for XPLMSetDatavi
	if (inPhase == xplm_CommandBegin && FuelPump[0] == 0 && XPLMGetDatai(d_battery_on))
	{
		FuelPump[1] = 1;
	}
	else if (inPhase == xplm_CommandContinue && FuelPump[0] == 0 && XPLMGetDatai(d_battery_on))
	{
		FuelPump[1] = 1;
	}
	else if (inPhase == xplm_CommandEnd || XPLMGetDatai(d_battery_on) == 0)
	{
		FuelPump[1] = 0;
	}
	XPLMSetDatavi(d_fuel_pump, FuelPump, 0, 8);
...
int gfe_annunciator_test_timeout_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if (inPhase == xplm_CommandBegin) 
	{
		timestamp_annunciators = GetTickCount();
	}
	return 0;
}
...
int gfe_gyr_free_slaved_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	...
	XPLMSetDatavi(d_gyr_free_slaved, gyr_free_slaved, 2, 1);
	return 0;
}
...
int gfe_dg_drift_ele_deg_up_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	if ((inPhase == xplm_CommandBegin) || (inPhase == xplm_CommandContinue))
	{
		XPLMSetDataf(d_dg_drift_ele_deg, XPLMGetDataf(d_dg_drift_ele_deg) + 1);
	}
	return 0;
}
...
float Get_d_gfe_elevtrimCB(void* inRefcon)
{
	return elev_trim_ind;
}

void Set_d_gfe_elevtrimCB(void* inRefcon, float inValue)
{
	elev_trim_ind = inValue;
}
...
void Set_d_gfe_rudtrimCB(void* inRefcon, float inValue)
{
	rud_trim_ind = inValue;
}

int Get_d_gfe_annunciatorCB(void* inRefcon)
{
	return annunciator_test_lit;
}

void Set_d_gfe_annunciatorCB(void* inRefcon, int inValue)
{
	annunciator_test_lit = inValue;
}
...
float CustomDatarefToDRECB(float elapsedMe, float elapsedSim, int counter, void * refcon)
{
	...
	if (PluginID != XPLM_NO_PLUGIN_ID)											//only run if DRE plugin is loaded
	{
		XPLMSendMessageToPlugin(PluginID, MSG_ADD_DATAREF, (void*)"gfe/elevtrimindicator");
		XPLMSendMessageToPlugin(PluginID, MSG_ADD_DATAREF, (void*)"gfe/ailtrimindicator");
		XPLMSendMessageToPlugin(PluginID, MSG_ADD_DATAREF, (void*)"gfe/rudtrimindicator");
		XPLMSendMessageToPlugin(PluginID, MSG_ADD_DATAREF, (void*)"gfe/annunciators");
	}
	return 0;																	//run only one time
}

//currently unused as BFF CL driver always needs two calls
...
//smooth lead-in of fuel gauges on battery on
//unfortunately, fuel indications are read-only, so we need to use actual fuel weights
//therefore fuel state must be saved before lead-in 
void smooth_fuel_gauge_Proc()
{
	int n = 0;
	float tmp_m_fuel[9] = { 0.0 };

	if (fuel_gauge_flag == 1)
	{
		gauge_leadin_ratio = gauge_leadin_ratio + 0.05;
		if (gauge_leadin_ratio > 1.0)
		{
			gauge_leadin_ratio = 1.0;
			fuel_gauge_flag = 0;
		}
		for (n = 0; n < 9; n++)
		{
			tmp_m_fuel[n] = gauge_leadin_ratio * mem_m_fuel[n];
		}
		XPLMSetDatavf(d_m_fuel, tmp_m_fuel, 0, 9);
	}
}
...
float init_sound(float elapsed, float elapsed_sim, int counter, void * ref)
{
	...
	// Make our context current, so that OpenAL commands affect our stuff.		
	alcMakeContextCurrent(my_ctx);

	XPLMDebugString("gfeSounds: created a context.\n");
	CHECK_ERR();
	...
	for (i=0;i<3;i++)
	{
		...
		alSourcef(snd_src[i],AL_GAIN,0.02f);	
