...
temperature and density altitude as well as altitude monitor and alert.
...
Configuration that is originally been done in the configuration menu is partially available in an INI file, editable with any text  editor.
Size of the instrument can be configured in this INI file as well. 
...
Disclaimer:
This software is only for recreational purposes. It is not intended to fully simulate each and every function of the original device.
It is not endorsed by the original manufacturer.
It has been tested on X-Plane 9 and 10 and is available in 32bit and 64bit versions.
For anyone interested in compiling a Mac or Linux version, full sourcecode is included (plain old C code)
...
- ADS-B
- Audio Alerts (still looking for these voices)
- Contrast Dimmer (no plane available for me with that mode activated)
...
- Alerts "Timer Expired" and "Leaving Altitude"
- Reply icon stays on after change from ALT to GND ?
- AltMonitor 
	what is displayed on > 1000ft + deviation

Work in pogress:
- recycle does not clear counters									<<==== check now, reworkee
- sound worked on
next: prepare for muliple sounds with 
...
- take Pilots Manual, photo, adapter for Headset out into iPhone Mic
- init run 
	duration
...
- Count Down Timer
	...
	- switch over to Count Up, or rather stays at 00:00:00 in CountDn ?
- get timers reset on power cycle ?
...
- photo and test Altitude Monitor 
...
#define A_ALTMON	"gtx_altalert_m.wav"								//male voice "leaving altitude"
#define A_TIMEXP	"gtx_timeexp_m.wav"									//male voice: "timer expired"
#define A_ALERT		"gtx_alert.wav"										//generic alert sound
                                   ...
                                   void *               inRefcon)
{
	...
	if (gfeDisplayPanelWindow)
	{
		...
		if ((AvnOn) && (fBat[0] > 21) && (BattOn) && (FSMode != M_OFF))	//required actual bat voltage minimum 21 Volts
		{
			if (FSMode == M_TEST) 										//Selftest sequence - Init page
			{
				DrawDisplay();
				DrawInit();												//init screen is always full bright						
			}
			else	
			{
				...
				if (CountDn == 0)										//on end of Count Down, start Count up
				{
					...
					//CountUpPause = 0;
					alSourcef(snd_src[1],AL_PITCH,pitch);				//play sound
...
void KeyIDENT(void * refCon)
{
	...
	alSourcef(snd_src[0],AL_PITCH,pitch);
...
void KeyVFR(void * refCon)
{
	...
	alSourcef(snd_src[1],AL_PITCH,pitch);
	alSourcePlay(snd_src[1]);
}
...
void KeyFUNC(void * refCon)
{
	...
	alSourcef(snd_src[2],AL_PITCH,pitch);
	...
	else if (nTick > 30)
	{
		if ((nTick % 4) == 0)											//every second check altitude deviation
		{
			...
			if ((abs(Deviation) > 100)	&& (abs(Deviation) < 1100))		//if 100 < altitude deviation < 1100ft
			{
				...
				alSourcef(snd_src[0],AL_PITCH,pitch);					//configure sound source
				alSourcePlay(snd_src[0]);								//play sound
			}
			else 
			{
				AltAlert = 0;
...
int OpenINIFile()
{
	...
	GetPrivateProfileString("common","OAT","",sOAT,255,INIFile);
	GetPrivateProfileString("common","FC","",sFC,255,INIFile);
	GetPrivateProfileString("common","AutoFlightTimer","",sAutoFlightTimer,255,INIFile);
	GetPrivateProfileString("common","Display","",sDisplay,255,INIFile);
	GetPrivateProfileString("common","Backlight","",sBacklight,255,INIFile);
	gaugeX1 = atof(sLeft);
	gaugeX2 = atof(sRight);
...
static float init_sound(float elapsed, float elapsed_sim, int counter, void * ref)
{
	...
	char filepath[255];
	char wavfile[3][30];															
	strcpy(wavfile[0], A_ALTMON);
	strcpy(wavfile[1], A_TIMEXP);
	strcpy(wavfile[2], A_ALERT);
