	...
	void *               inRefcon)
{
	...
	if (FLCB == 1)
	{
		...
		XPLMSetDatai(d_avionics_power_on, 0);										//still starts with on
	...
	//smooth lead-in or lead-out of fuel gauges
...
int gfe_battery_Proc(XPLMCommandRef inCommand, XPLMCommandPhase inPhase, void *inRefcon)
{
	...
	if ((inPhase == xplm_CommandBegin) || (inPhase == xplm_CommandContinue))
	{
		...
		smooth_fuel_gauge_Proc();
		//fuel_gauge_flag = 3;													//no lead-in or lead-out
	}
	else
	{
		...
		//XPLMGetDatavf(d_m_fuel, mem_m_fuel, 0, 9);							//save current fuel state
		//fuel_gauge_flag = 2;													//lead-out fuel gauge
	}
	return 0;
}
...
//unfortunately, fuel indications are read-only, so we need to use with the actual fuel weights
//therefore fuel state must be saved before and reset after lead-in and -out operations
void smooth_fuel_gauge_Proc()
{
	int n = 0;
	...
	else if (fuel_gauge_flag == 2)													//smooth fade down of fuel indications
	{
		/*
		//leads out to zero fuel, so previous qantity would need to be restored
		//but then, 

		gauge_leadin_ratio = gauge_leadin_ratio - 0.05;							
		if (gauge_leadin_ratio < 0.0)
		{
			gauge_leadin_ratio = 0.0;
			fuel_gauge_flag = 0;
		}
	...
	else if (fuel_gauge_flag == 3)
	{
		//battery on, fuel gauge indicates correctly, do nothing
	}
	else
	{
		/*
		//rebuild actual fuel state
		//issue here is that fuel qty stays constant when battery off, even when engine continues turning!
		XPLMSetDatavf(d_m_fuel, mem_m_fuel, 0, 9);								
		*/
	}
}
