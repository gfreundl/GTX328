...
int iOIL;									//number of segments to display 
						...
						char *		outDesc)
{
	...
	dxplEGT = XPLMFindDataRef("sim/flightmodel/engine/ENGN_EGT_c");										//temperatures in °F though
	...
	gHotKeyLF = XPLMRegisterHotKey(XPLM_VK_F2, xplm_DownFlag,	"F2 LF", HotKeyLFCallback, NULL);
	gHotKeyLFHold = XPLMRegisterHotKey(XPLM_VK_F2, NULL,	"F2 LF Hold", HotKeyLFHoldCallback, NULL);
                                   ...
                                   void *               inRefcon)
{
	...
	if (gfeDisplayPanelWindow)
	{
		...
		if (FirstCall)															//first Call since Plugin Start or Battery Switch on/off
		{
			...
			{
				OpenINIFile();													
				fRemFuel = StartFuel;											//we receive StartFuel from INI file in correct units
			}
			else
			{
				StartFuel =	XPLMGetDataf(dFuelTotal) * fFuelFactor;				//DataRef unit is kg, so we need to convert
			}				
			FirstCall = FALSE;
			FSMode = M_TEST;
			FSStep = M_TEST_1;
			...
			b = 0;
			c = 0;
			nFuelAlarm = 0;														//uncertain if real thing might not save alarm state on power cycle
			nTimeAlarm = 0;
		}
		else
		{
			nTick = nTick;														//dummy for debug
		}
		...
		if (BattOn)
		{
			...
			if ((FSMode == M_TEST) && (FSStep == M_TEST_1))							//Selftest sequence - all symbols on
			{
				SelfTest_1();																		
			}
			else if ((FSMode == M_TEST) && (FSStep == M_TEST_2))					//Selftest sequence - columns
			{
				SelfTest_2();
			}
			else if (((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && (FSStep == M_BAT))
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
				sprintf(cText1,"%4.0f",fBat[0]*10);		
				DrawLeftDigit();
				strcpy(cText2, "BAT");
				DrawRightDigit();			
			}
			else if (((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && (FSStep == M_OAT))
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
				sprintf(cText1,"%3.0f",fOAT*1.8 + 32);		
				DrawLeftDigit();
				strcpy(cText2, "OAT");
				DrawRightDigit();
			}
			else if (((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && (FSStep == M_IAT))
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
			...
			else if (((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && (FSStep == M_CDT))
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
			...
			else if (((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && (FSStep == M_CRB))
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
			...
			else if (((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && (FSStep == M_DIF))
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
			...
			else if (((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && (FSStep == M_OIL))
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
				sprintf(cText1,"%3.0f",fxplOIL[0]);		
				DrawLeftDigit();
				strcpy(cText2, "OIL");
				DrawRightDigit();
			}
			else if (((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && (FSStep == M_CLD))
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
			...
			else if ( ((FSMode == M_AUTOSEQ) || (FSMode == M_MANSEQ)) && ((FSStep >= M_CYL1) && (FSStep <= M_CYL6)) )
			{
				DrawRPM();
				DrawHeadlines();
				DrawBars();
                                   ...
                                   void *               inRefcon)
{
	...
	{		
		FSMode = M_TEST;
		FSStep = M_TEST_1;														//draw symbols test, no columns
	}
	else if (nTick < 30)
	{		
		FSMode = M_TEST;
		FSStep = M_TEST_2;														//draw columns test
	}
	else if (nTick == 30)
	{
		FSStep = M_CYL1;
...
void DrawRPM()															//draw horsepower or RPM digits
{
	if (HPOption == 1)
	{
...
void DrawRightDigit()
{
	for  (i=1; i<4; i++)												//right digits
	{
		a = GetTextureID(cText2[i-1]);
		PaintTexture(a, SymbolWidth, PanelL + DigitL + 2*SymbolWidth + i*SymbolWidth*0.6, PanelB + DigitB);
	}
