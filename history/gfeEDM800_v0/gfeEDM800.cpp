...
int IndexRate;								//Index Rate in Auto Index Mode [seconds]
                                   ...
                                   void *               inRefcon)
{
	...
	if (gfeDisplayPanelWindow)
	{
		...
		// Original Panel Size = 256x256, Symbol Textures 32x32, so scale everything accordingly
		Scaler = (PanelR-PanelL)/256;

		// Row Coordinates
		SymbolWidth = SymbolWidth * Scaler;								//Seven Segment Glyphs
		//DecWidth = floor(0.7 * DecWidth * Scaler);						//Decimal Point: 4 bit too small, 8 bit too large ...
		...
		float FFRowLeft = PanelL + floor(0.28 * (Scaler * 256));		//70,62 in Panel.bmp
		float FFRowBottom = PanelB + floor(0.6 * (Scaler * 256)); 
		float FFRowTop = FFRowBottom + SymbolWidth; 
		...
		// draw headlines
		n = 0;
		XPLMBindTexture2d(gTexture[CYL1_TEXTURE], 0);
		glBegin(GL_QUADS);
				glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4 + HdWidth, PanelB +  HeaderB);		// Bottom Right 
				glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HeaderB);				// Bottom Left 		
				glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HdWidth + HeaderB);			// Top Left 
				glTexCoord2f(1, 1.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4 + HdWidth, PanelB +  HdWidth + HeaderB);	// Top Right 
		glEnd();
		n = 1;
		XPLMBindTexture2d(gTexture[CYL2_TEXTURE], 0);
		glBegin(GL_QUADS);
				glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4 + HdWidth, PanelB +  HeaderB);		// Bottom Right 
				glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HeaderB );				// Bottom Left 		
				glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HdWidth + HeaderB);			// Top Left 
		...
		glEnd();
		n = 2;
		XPLMBindTexture2d(gTexture[CYL3_TEXTURE], 0);
		glBegin(GL_QUADS);
				glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4 + HdWidth, PanelB +  HeaderB);		// Bottom Right 
				glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HeaderB );				// Bottom Left 		
				glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HdWidth + HeaderB);			// Top Left 
		...
		glEnd();
		n = 3;
		XPLMBindTexture2d(gTexture[CYL4_TEXTURE], 0);
		glBegin(GL_QUADS);
				glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4 + HdWidth, PanelB +  HeaderB);		// Bottom Right 
				glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HeaderB );				// Bottom Left 		
				glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HdWidth + HeaderB);			// Top Left 
		...
		glEnd();
		n = 4;
		XPLMBindTexture2d(gTexture[CYL5_TEXTURE], 0);
		glBegin(GL_QUADS);
				glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4 + HdWidth, PanelB +  HeaderB);		// Bottom Right 
				glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HeaderB );				// Bottom Left 		
				glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HdWidth + HeaderB);			// Top Left 
		...
		glEnd();
		n = 5;
		XPLMBindTexture2d(gTexture[CYL6_TEXTURE], 0);
		glBegin(GL_QUADS);
				glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4 + HdWidth, PanelB +  HeaderB);		// Bottom Right 
				glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HeaderB );				// Bottom Left 		
				glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*HdWidth*1.4, PanelB +  HdWidth + HeaderB);			// Top Left 
		...
		for (n = 0; n < 6; n++)														//step through cylinders
		{
			...
			if (iEGT[n] > 0) 
			{
				for (i = 0; i <= iEGT[n]; i++)
				{
					XPLMBindTexture2d(gTexture[LBAR_TEXTURE], 0);
					glBegin(GL_QUADS);
							glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*BarWidth*2.4 + BarWidth, PanelB + BarB + i*(0.5*BarWidth));		// Bottom Right 
							glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*BarWidth*2.4, PanelB + BarB + i*(0.5*BarWidth) );				// Bottom Left 		
							glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*BarWidth*2.4, PanelB + BarB + BarWidth + i*(0.5*BarWidth));			// Top Left 
							glTexCoord2f(1, 1.0f); glVertex2f(PanelL + BarL + n*BarWidth*2.4 + BarWidth, PanelB + BarB + BarWidth + i*(0.5*BarWidth));	// Top Right 
					glEnd();
					XPLMBindTexture2d(gTexture[RBAR_TEXTURE], 0);
					glBegin(GL_QUADS);
							glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*BarWidth*2.4 + BarWidth + BarWidth, PanelB + BarB + i*(0.5*BarWidth));		// Bottom Right 
							glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*BarWidth*2.4 + BarWidth, PanelB + BarB + i*(0.5*BarWidth) );				// Bottom Left 		
							glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*BarWidth*2.4 + BarWidth, PanelB + BarB + BarWidth + i*(0.5*BarWidth));			// Top Left 
							glTexCoord2f(1, 1.0f); glVertex2f(PanelL + BarL + n*BarWidth*2.4 + BarWidth + BarWidth, PanelB + BarB + BarWidth + i*(0.5*BarWidth));	// Top Right 
					glEnd();
...
/*			//display CHT
			...
			{
				i = iCHT[n] - 1;
				XPLMBindTexture2d(gTexture[GAP_TEXTURE], 0);
				if (iCHT[n] >= iEGT[n])
				{
					XPLMBindTexture2d(gTexture[GAP2_TEXTURE], 0);
				}
				glBegin(GL_QUADS);
						glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL + n*BarWidth*1.4 + BarWidth, PanelB + BarB + i*(0.35*BarWidth));		// Bottom Right 
						glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL + n*BarWidth*1.4, PanelB + BarB + i*(0.35*BarWidth) );				// Bottom Left 		
						glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL + n*BarWidth*1.4, PanelB + BarB + BarWidth + i*(0.35*BarWidth));			// Top Left 
						glTexCoord2f(1, 1.0f); glVertex2f(PanelL + BarL + n*BarWidth*1.4 + BarWidth, PanelB + BarB + BarWidth + i*(0.35*BarWidth));	// Top Right 
				glEnd();	
			}			
*/
		}
...
*/
		...
		if ((FSStep >= M_CYL1) && (FSStep <= M_OIL))
		{
			...
			for  (i=0; i<4; i=i+1)
			{
				...
				XPLMBindTexture2d(gTexture[a], 0);
				glBegin(GL_QUADS);
						glTexCoord2f(1, 0.0f); glVertex2f(PanelL + DigitL + i*SymbolWidth*0.6 + SymbolWidth, PanelB + DigitB);		// Bottom Right 
						glTexCoord2f(0, 0.0f); glVertex2f(PanelL + DigitL + i*SymbolWidth*0.6, PanelB + DigitB);				// Bottom Left 		
						glTexCoord2f(0, 1.0f); glVertex2f(PanelL + DigitL + i*SymbolWidth*0.6, PanelB + DigitB + SymbolWidth);			// Top Left 
						glTexCoord2f(1, 1.0f); glVertex2f(PanelL + DigitL + i*SymbolWidth*0.6 + SymbolWidth, PanelB + DigitB + SymbolWidth);	// Top Right 
				glEnd();
			}
			sprintf(cText1,"%4.0f",fCHT[n]);
			for  (i=1; i<4; i=i+1)
			{
				a = GetTextureID(cText1[i]);
				XPLMBindTexture2d(gTexture[a], 0);
				glBegin(GL_QUADS);
						glTexCoord2f(1, 0.0f); glVertex2f(PanelL + DigitL + 1.9*SymbolWidth + i*SymbolWidth*0.6 + SymbolWidth, PanelB + DigitB);		// Bottom Right 
						glTexCoord2f(0, 0.0f); glVertex2f(PanelL + DigitL + 1.9*SymbolWidth + i*SymbolWidth*0.6, PanelB + DigitB);				// Bottom Left 		
						glTexCoord2f(0, 1.0f); glVertex2f(PanelL + DigitL + 1.9*SymbolWidth + i*SymbolWidth*0.6, PanelB + DigitB + SymbolWidth);			// Top Left 
						glTexCoord2f(1, 1.0f); glVertex2f(PanelL + DigitL + 1.9*SymbolWidth + i*SymbolWidth*0.6 + SymbolWidth, PanelB + DigitB + SymbolWidth);	// Top Right 
				glEnd();
			}
			...
			{
				XPLMBindTexture2d(gTexture[CYL_DOT_TEXTURE], 0);
				glBegin(GL_QUADS);
						glTexCoord2f(1, 0.0f); glVertex2f(PanelL + BarL*1.2 + n*BarWidth*1.4 + DotWidth, PanelB + HeaderB*0.95);		// Bottom Right 
						glTexCoord2f(0, 0.0f); glVertex2f(PanelL + BarL*1.2 + n*BarWidth*1.4, PanelB +  HeaderB*0.95);				// Bottom Left 		
						glTexCoord2f(0, 1.0f); glVertex2f(PanelL + BarL*1.2 + n*BarWidth*1.4, PanelB +  DotWidth + HeaderB*0.95);			// Top Left 
						glTexCoord2f(1, 1.0f); glVertex2f(PanelL + BarL*1.2 + n*BarWidth*1.4 + DotWidth, PanelB +  DotWidth + HeaderB*0.95);	// Top Right 
				glEnd();
			}
		}
