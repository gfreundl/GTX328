...
014     custom commands for buttons, 64bit version for X-Plane 11
015		fixed uninitialized variable iTrend
016		deactivate device when failure DataRef triggered sim/operation/failures/rel_xpndr
                ...
                void *               inRefcon)
{
	...
	if (gfeDisplayPanelWindow)
	{
		...
		//only show when Avionics on, battery voltage and not failed
		//required actual bat voltage minimum 7 or 19 Volts
		if ((AvnOn) && (fBat[0] > (Voltage-5)) && (BattOn) && (FSMode != M_OFF) && !(Fail ==6))	
		{
