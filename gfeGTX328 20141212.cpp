...
Configuration that is originally been performed in the configuration menu is partially available in a config file, editable with any text editor.
...
Plugin has been tested on X-Plane 9.40 and 10.30, currently as 32bit only. Platform is Windows only.
...
For anyone interested in compiling a Mac or Linux version, full sourcecode is included (plain simple C code)
...
Open Issues:
- OAT sketchy
- switch to STBY or GND on landing, default delay 24sec
- FL show "___" in GND and STBY after some time
- audio alerts "Timer expired"  "Leaving Altitude"
- FLT TIME saved on reset (for a limited time)
- Count Down Timer
	- maximum start value 99:59:59
	- switch over to Count Up at 00:00:00 with "EXPIRED" independent from CountDn Timer
	- invert current character on enter
- FL trend thresholds currently just guesswork

Notes: 
bezel size in px for display size 198x33:  431x114, 680x179 for aspect ratio 3.8 (original size 42mm/159mm)
all less black
...
float Red = 0;								//X-Plane ambint light
float Green = 0;
float Blue = 0;
...
void DrawDisplay()
{
	...
	else
	{
		...
		PaintTexture2(DISPLAY_TEXTURE, 0, 0, DispL, DispB);
		glColor3f(0, 0, 0);												//switch back to black characters
	}	
}
...
int FLTrend()
{
	...
	if ((fTrendArrow >= -0.3) && (fTrendArrow <= 0.3))		iTrend = 0;			
	if ((fTrendArrow > 0.3) && (fTrendArrow < 2.0))			iTrend = 1;
	if (fTrendArrow >= 2.0)									iTrend = 2;
	if ((fTrendArrow < -0.3) && (fTrendArrow > -2.0))		iTrend = -1;
	if (fTrendArrow < -2.0)									iTrend = -2;
...
void DrawIdent()
{
	...
	for (i=0; i < (strlen(c)); i++)
	{
		...
		PaintTexture2(a, 0, 0, DispL + offset, 1.14 * DispB);
	}
}
