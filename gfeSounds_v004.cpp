...
float joy_mapped_axis_value[100] = { 0.0 };
int fuel_pump;
int battery_on;
...
float	call_ext_FlightLoopCallback(
    ...
    int                  inCounter,    
    void *               inRefcon);    
int    gfe_radios_vol_up_Proc(
	XPLMCommandRef        inCommand,
	XPLMCommandPhase      inPhase,
	void *                inRefcon);
int    gfe_radios_vol_dn_Proc(
	XPLMCommandRef        inCommand,
	XPLMCommandPhase      inPhase,
	void *                inRefcon);
...
PLUGIN_API int XPluginStart(char * name, char * sig, char * desc)
{
	...
	d_joy_mapped_axis_value = XPLMFindDataRef("sim/joystick/d_joy_mapped_axis_value");
	...
	void *               inRefcon)
{
	...
	i = XPLMGetDatavf(d_joy_mapped_axis_value, joy_mapped_axis_value, 0, 100);
	...
	//joy_mapped_axis_value[6] = joy_mapped_axis_value[6] / 20 * pow(20, joy_mapped_axis_value[6]);
	//joy_mapped_axis_value[7] = joy_mapped_axis_value[7] / 20 * pow(20, joy_mapped_axis_value[7]);
	...
	a[0] = joystick_button_values[981];
	XPLMSetDatavi(d_generator_on, a,0,1);
...
int    gfe_radios_vol_up_Proc(	XPLMCommandRef       inCommand,
							XPLMCommandPhase     inPhase,
							void *               inRefcon)
{
	if (inPhase == xplm_CommandEnd)
	{
		if (XPLMGetDataf(d_radio_volume_ratio) <= 0.98)
		{
			XPLMSetDataf(d_radio_volume_ratio, XPLMGetDataf(d_radio_volume_ratio) + 0.02);
		}
	}
	return 0;
}

int    gfe_radios_vol_dn_Proc(XPLMCommandRef       inCommand,
	XPLMCommandPhase     inPhase,
	void *               inRefcon)
{
	if (inPhase == xplm_CommandEnd)
	{
		if (XPLMGetDataf(d_radio_volume_ratio) >= 0.02)
		{
			XPLMSetDataf(d_radio_volume_ratio, XPLMGetDataf(d_radio_volume_ratio) - 0.02);
		}
	}
